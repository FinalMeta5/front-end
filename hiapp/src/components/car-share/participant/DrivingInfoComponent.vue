<template>
  <div class="taxi-share-list">
      ...
      <div v-if="drivingInfo && Object.keys(drivingInfo).length > 0">
        <div v-if="drivingInfo.imageUrl">
            <img id="driver-img" :src="drivingInfo.imageUrl" alt="Car Image" @click="openImageModal(drivingInfo.imageUrl)"/>
        </div>
        <p>{{ drivingInfo.pickupDate }}</p>
        <p>{{ drivingInfo.passengersNum }}</p>
        <p>{{ drivingInfo.leftoverNum}}</p>
      </div>
...
import axios from 'axios';
  ...
  export default {
    ...
    this.drivingInfo = response.data;  // 응답 데이터 처리
  } catch (error) {
    ...
    mounted() {
      if (this.driverId) {
        this.fetchDrivingInfo(this.driverId, this.carShareRegiId);  
      }
    },
  };
</script>
